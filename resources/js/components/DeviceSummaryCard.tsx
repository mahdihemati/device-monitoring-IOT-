import { ArrowLeft, BellRing, Clock3, Hash, ShieldCheck, Thermometer } from 'lucide-react';
import { Link } from 'react-router-dom';
import type { Device } from '../types';
import { formatDateTime, formatTemperature } from '../utils/format';
import { alarmCountText } from '../utils/localization';
        ...
        <article className={`flex h-full flex-col overflow-hidden rounded-lg border border-r-4 border-slate-200 bg-white shadow-sm shadow-slate-200/70 transition hover:-translate-y-0.5 hover:shadow-md ${statusAccentStyles[status.level]}`}>
            ...
            <div className="mt-auto space-y-3 p-4">
                ...
                <Link
                    ...
                    className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-lg border border-sky-200 bg-sky-50 px-4 text-sm font-semibold text-sky-700 transition hover:bg-sky-100 focus:outline-none focus:ring-4 focus:ring-sky-100"
                >
                    مشاهده جزئیات
